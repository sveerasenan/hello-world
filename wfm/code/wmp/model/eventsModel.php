...
class eventsModel extends BasePDOMySQLModel {
    ...
    public function addEvent($userId){
		...
		try {
			...
			$sql="INSERT INTO `EOC_EVENT` (EOC_ENT_ID,EOC_EVNT_DATE,`EOC_EVNT_START_TIME`,`CHANGE_DATE`,`CHANGE_USER`,`CHANGE_TYPE`) VALUES (NULL,now(),now(),now(),:CHANGE_USER,
		    'I');" ;
...
public function updateStatus($params,$changeuser){
  ...
  public function getEventStatus($event_id){
	...
	//pagelimit -- number of records to fetch
	//pgoffset -- starting record to fetch for range fetches.
    public function getEmployeesByStatus($event_id,$emp_id,$status,$pagelimit,$pgoffset){
		...
		try
		{
			if(empty($status))
			{
			$sql="SELECT * FROM EOC_EMPLOYEE_EVENT WHERE ORG_EMPL_STATUS='A' AND EOC_ENT_ID=:eventid AND ORG_MGR_ID_TREE like :mgr_id_hash AND (EOC_EVENT_EMP_STATUS is NULL or EOC_EVENT_EMP_STATUS='') order by ORG_EMPL_LNAME,ORG_EMPL_FNAME LIMIT ".$pagelimit.' OFFSET '.$pgoffset;
				$input[':eventid'] =  $event_id;
				$input[':mgr_id_hash'] =  "%M" . $emp_id . "M%";
				
			}
			else {
				$sql="SELECT * FROM EOC_EMPLOYEE_EVENT WHERE ORG_EMPL_STATUS='A' AND EOC_ENT_ID=:eventid AND ORG_MGR_ID_TREE like :mgr_id_hash AND EOC_EVENT_EMP_STATUS=:status order by ORG_EMPL_LNAME,ORG_EMPL_FNAME LIMIT ".$pagelimit.' OFFSET '.$pgoffset;
				$input[':eventid'] =  $event_id;
				$input[':mgr_id_hash'] =  "%M" . $emp_id . "M%";
				$input[':status'] =  $status;
    ...
    public function getEmployeesCountByStatus($event_id,$emp_id,$status){
		...
		try
		{
			if(empty($status))
			{
			$sql="SELECT * FROM EOC_EMPLOYEE_EVENT WHERE ORG_EMPL_STATUS='A' AND EOC_ENT_ID=:eventid AND ORG_MGR_ID_TREE like :mgr_id_hash AND (EOC_EVENT_EMP_STATUS is NULL or EOC_EVENT_EMP_STATUS='') order by ORG_EMPL_LNAME,ORG_EMPL_FNAME ";
				$input[':eventid'] =  $event_id;
				$input[':mgr_id_hash'] =  "%M" . $emp_id . "M%";
